...
export default function AllocationChart({
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-card border-border rounded-md border p-3 shadow-lg">
    ...
    <Card className="bg-neutral-950">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <span className="text-white">Portfolio Allocation</span>
          <span className="text-lg text-white">{totalValue}</span>
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className="h-64">
